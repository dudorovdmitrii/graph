...
export class GraphTask3 extends Graph {
  ...
  printResult({ bridges, articulationPoints }: PrintResult) {
    console.log("Bridges:");
    console.log(
      bridges.map((bridge) => bridge.map((vertex) => shiftVertex(vertex)))
    );
    console.log("Cut vertices:");
    console.log(articulationPoints.map((vertex) => shiftVertex(vertex)));
  }
}
