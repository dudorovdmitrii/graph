...
export class GraphTask5 extends Graph {
  ...
  solveByDijkstra(start: number, end: number) {
    const distances: number[] = new Array(this.length).fill(Number.MAX_SAFE_INTEGER); // массив расстояний до каждой вершины
  ...
  printOrWriteResult({ distance, path }: PrintResult, start: number, end: number) {
    ...
    writeOrPrintMatrix({
      matrix: path.map(([start, end, weight]) => [shiftVertex(start), shiftVertex(end), weight]),
      before: 'Path',
      stream,
    });
