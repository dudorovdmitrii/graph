...
export class GraphTask4 extends Graph {
  ...
  solveByPrim() {
    ...
    const minKey = (key: Array<number>, mstSet: Array<boolean>, length: number): number => {
    ...
    const primMST = () => {
      ...
      for (let i = 0; i < this.length - 1; i++) {
        ...
        for (let v = 0; v < this.length; v++) {
          if (this.matrix[u][v] != 0 && mstSet[v] == false && this.matrix[u][v] < key[v]) {
            parent[v] = u;
  ...
  solveByBuravka() {
    ...
    while (new Set(components).size !== 1) {
      const cheapest: Cheapest[] | null = new Array(this.length).fill(null);
      ...
      for (let i = 0; i < this.length; ++i) {
        if (cheapest[i] !== null) {
          ...
          if (components[source] !== components[target]) {
            tree.push([cheapest[i].source, cheapest[i].target, cheapest[i].weight]);
            cost += weight;
  ...
  solveByKruscal() {
    ...
    const tree_id: number[] = new Array(this.length).fill(0).map((_, ind) => ind);
  ...
  printOrWriteResult(results: PrintResult[], time: number[]) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      writeOrPrintMatrix({
        matrix: tree.map(([from, to, weight]) => [shiftVertex(from), shiftVertex(to), weight]),
